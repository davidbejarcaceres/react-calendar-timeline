...
exports.getGroupWithItemDimensions = getGroupWithItemDimensions;
exports.groupItemsByKey = groupItemsByKey;
exports.getOrderedGroupsWithItems = getOrderedGroupsWithItems;
exports.shallowIsEqualOrderedGroup = shallowIsEqualOrderedGroup;
exports.isEqualItemWithDimensions = isEqualItemWithDimensions;
exports.getGroupsWithItemDimensions = getGroupsWithItemDimensions;
exports.getGroupsCache = exports.minCellWidth = void 0;
...
function groupStack(lineHeight, item, group, groupHeight, itemIndex) {
  ...
  if (item.dimensions.stack && item.dimensions.top === null) {
    ...
    do {
      ...
      if (collidingItem != null) {
        ...
        curHeight = Math.max(curHeight, item.dimensions.top + item.dimensions.height + verticalMargin);
      }
    } while (collidingItem);
  }
...
function groupNoStack(lineHeight, item, groupHeight) {
  var verticalMargin = (lineHeight - item.dimensions.height) / 2;
...
function stackGroup(itemsDimensions, isGroupStacked, lineHeight) {
  var groupHeight = 0;
  ...
  for (var itemIndex = 0; itemIndex < itemsDimensions.length; itemIndex++) {
    ...
    if (isGroupStacked) {
      r = groupStack(lineHeight, itemsDimensions[itemIndex], itemsDimensions, groupHeight, itemIndex);
    } else {
  ...
  return {
    groupHeight: groupHeight || lineHeight,
    verticalMargin: verticalMargin
  };
}
/**
...
function stackTimelineItems(items, groups, canvasWidth, canvasTimeStart, canvasTimeEnd, keys, lineHeight, itemHeightRatio, stackItems, draggingItem, resizingItem, dragTime, resizingEdge, resizeTime, newGroupId) {
  ...
  var groupHeights = groups.map(function (group) {
    var groupKey = (0, _generic._get)(group, keys.groupIdKey);
    var groupsWithItemDimensions = groupsWithItemsDimensions[groupKey];
    return groupsWithItemDimensions.height;
  });
  var groupTops = groupHeights.reduce(function (acc, height, index) {
    ...
    var lastTop = acc[lastIndex];
    acc.push(lastTop + height);
    return acc;
  }, [0]);
  var height = groupHeights.reduce(function (acc, height) {
    return acc + height;
  }, 0);
  return {
...
function getItemWithInteractions(_ref4) {
  ...
  var newItem = _objectSpread({}, item, (_objectSpread2 = {}, _defineProperty(_objectSpread2, keys.itemTimeStartKey, itemTimeStart), _defineProperty(_objectSpread2, keys.itemTimeEndKey, itemTimeEnd), _defineProperty(_objectSpread2, keys.itemGroupKey, isDragging ? newGroupId : (0, _generic._get)(item, keys.itemGroupKey)), _objectSpread2));
...
function getGroupWithItemDimensions(groupWithItems, keys, canvasTimeStart, canvasTimeEnd, canvasWidth, lineHeight, itemHeightRatio, stackItems) {
  var itemDimensions = groupWithItems.items.map(function (item) {
  ...
  var _stackGroup = stackGroup(itemDimensions, stackItems, lineHeight),
      groupHeight = _stackGroup.groupHeight;

  return _objectSpread({}, groupWithItems, {
...
function groupItemsByKey(items, key) {
  return items.reduce(function (acc, item) {
    var itemKey = (0, _generic._get)(item, key);
...
function getOrderedGroupsWithItems(groups, items, keys) {
  var groupOrders = getGroupOrders(groups, keys);
  var groupsWithItems = {};
  var groupKeys = Object.keys(groupOrders);
  var groupedItems = groupItemsByKey(items, keys.itemGroupKey); // Initialize with result object for each group

  for (var i = 0; i < groupKeys.length; i++) {
    var groupOrder = groupOrders[groupKeys[i]];
    groupsWithItems[groupKeys[i]] = {
      index: groupOrder.index,
      group: groupOrder.group,
      items: groupedItems[(0, _generic._get)(groupOrder.group, keys.groupIdKey)] || []
    };
  }
...
function isEqualItemWithDimensions(newArgs, oldArgs) {
...
var getGroupsCache = (0, _memoizeOne["default"])(function (groups, keys, method) {
  return groups.reduce(function (acc, group) {
    var id = (0, _generic._get)(group, keys.groupIdKey);
    acc[id] = (0, _memoizeOne["default"])(method, isEqualItemWithDimensions);
    return acc;
  }, {});
});
exports.getGroupsCache = getGroupsCache;
...
function getGroupsWithItemDimensions(groupsWithItems, keys, lineHeight, itemHeightRatio, stackItems, canvasTimeStart, canvasTimeEnd, canvasWidth, groups) {
  var cache = getGroupsCache(groups, keys, getGroupWithItemDimensions);
